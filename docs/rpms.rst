...
:ref:`snapshot distribution <distribution-job-builds>` from a given branch as an RPM. This could be
used by a developer to test code that was just merged, but which has not been
included in an :ref:`autorelease builds <autorelease-builds>` yet. The job is also triggered daily,
