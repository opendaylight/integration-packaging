#!/usr/bin/env python
"""Build debian files from debian build config and Jinja2 debian templates."""
...
# per-build
odl_dir_template = Template("opendaylight/opendaylight-$version_major."
                            "$version_minor.$version_patch-$pkg_version/")
odl_deb_template = Template("opendaylight/opendaylight_$version_major."
                            "$version_minor.$version_patch-"
                            "${pkg_version}_all.deb")
...
def build_debfiles(build):
    ...
    unitfile_tarball_path = cache.cache_odl_unitfile(build)
    tar = tarfile.open(unitfile_tarball_path)
    tar.extractall(path=debian_dir_path)
    for filename in os.listdir(debian_dir_path):
        print filename, debian_dir_path
        if filename.endswith(".service"):
            src_service_file = os.path.join(debian_dir_path, filename)
            dest_service_file = os.path.join(debian_dir_path,
                                            "opendaylight.service")
            os.rename(src_service_file, dest_service_file)
    tar.close()
