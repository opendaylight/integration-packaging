...
distro_tar_template = distro_template + ".tar.gz"
distro_zip_template = distro_template + ".zip"
unitfile_tar_template = unitfile_template + ".tar.gz"

# Prepend cache dir path to get template of full path to cached zip/tarball
distro_tar_path_template = os.path.join(cache_dir, distro_tar_template)
distro_zip_path_template = os.path.join(cache_dir, distro_zip_template)
unitfile_path_template = os.path.join(cache_dir, unitfile_template)
unitfile_tar_path_template = os.path.join(cache_dir, unitfile_tar_template)
...
def cache_distro(build):
    ...
    distro_tar = distro_tar_template.substitute(build)
    distro_zip = distro_zip_template.substitute(build)
    distro_tar_path = distro_tar_path_template.substitute(build)
    distro_zip_path = distro_zip_path_template.substitute(build)
    ...
    if not os.path.isfile(distro_tar_path):
        ...
        print("Already cached: {}".format(distr_tar))
...
def cache_sysd(build):
    ...
    unitfile_path = unitfile_path_template.substitute(build)
    unitfile_tar = unitfile_tar_template.substitute(build)
    unitfile_tar_path = unitfile_tar_path_template.substitute(build)
