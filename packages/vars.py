...
##############################################################################
""" Retrieves variables needed for builds"""
...
def extract_version(url):
    ...
    :return dict version: A dictionary containing major version, minor version,
    patch version, package version, and codename extracted from URL.
    """
    if "autorelease" in url:
        # Autorelease URL does not include a date and hence date extraction
        # logic is needed for packaging.
        ...
        odl_pkg = re.search(
            ...
            url)
        pkg_version = '0.1.' + odl_pkg.group(3) + "snap" + odl_pkg.group(5)
...
def get_snap_url(version_major, version_minor):
    ...
    :arg str version_major: Minor version for snapshot build (optional)
    :return str snapshot_url: URL of the snapshot release
    """
    ...
    snapshot_dir = ''
    if version_minor:
        sub_dir = '0.' + version_major + '.' + \
                   version_minor + '-SNAPSHOT/'
        ...
        for tag in all_dirs.find_all('a', href=True):
            ...
            # store the latest sub-directory.
            if dir:
...
def get_sysd_commit():
    ...
    :return str sysd_commit: Commit hash at the tip of the master branch
    """
...
def get_java_version(version_major):
    """Get the java_version dependency for ODL build (for debian only)
       TODO : Improve logic for this function
