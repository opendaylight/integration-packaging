...
##############################################################################
# Copyright (c) 2016 Integration/Packaging.  All rights reserved.
#
...
import re
import shutil
<<<<<<< fd5a81853e71d45945471d0f91bbdac1a1444386
import subprocess
import datetime
import re
...
def extract_version(url):
    ...
    odl_date = subprocess.Popen(
        ...
        "| sed \"s/\\n//g\" | awk '{print $3,$2,$6}' ", shell=True, stdout=subprocess.PIPE,
    ...
    if "autorelease" in url:
        ...
        # date = 20161201
        odl_rpm = re.search(
            r'\/(distribution-karaf)-([0-9]\.[0-9]\.[0-9])-([0-9]+)\.([0-9]+)-([0-9]+)\.(tar\.gz)',
            url)
